...
git clone https://github.com/ukondert/pr_digital-urbanbloom.git
cd pr_digital-urbanbloom
...
java -jar target/server-app-0.0.1-SNAPSHOT.jar
...
```bash
# Location: backend/server-app/src/main/resources/db/migration/
...
|---------|------|---------|---------|
| **Lending** | Core Domain | `com.urbanbloom.lending` | Checkout, Return, Renewal, Reservation |
| **Catalog** | Supporting | `com.urbanbloom.catalog` | Media Management, Inventory |
| **Notification** | Supporting | `com.urbanbloom.notification` | Event-Driven Notifications |
| **Reminding** | Supporting | `com.urbanbloom.reminding` | Scheduled Reminders |
| **User** | Generic | `com.urbanbloom.user` | User Management, Keycloak Integration |
...
DB_HOST=localhost
DB_PORT=5432
DB_NAME=urbanbloom_db
DB_USER=dev_user
DB_PASSWORD=dev_password

KEYCLOAK_URL=http://localhost:8081
KEYCLOAK_REALM=urbanbloom
KEYCLOAK_CLIENT_ID=server-app
...
docker exec urbanbloom-postgres psql -U dev_user -d urbanbloom_db -c "\dt"

# Check application logs
tail -f backend/server-app/logs/application.log
```
