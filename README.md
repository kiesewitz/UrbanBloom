...
Runnables "Hello World" mono-repo for infrastructure verification (Backend + Web Admin + Mobile).

## Hello World Quick Start (<= 10 min)
...
- Keycloak Admin UI: http://localhost:8081 (default: `admin` / `admin`)
- **Wichtig:** Folge der [Keycloak Setup Anleitung](docs/configurations/keycloak-setup.md) für die initiale Konfiguration des Realms und der Rollen.
...
- `GET http://localhost:8080/api/v1/app/info`

### 4) Start frontend web (admin)
...
### 5) Start frontend mobile (Flutter)
